...
/// Create stateless routes for the Schedule module.
///
...
///
/// ```ignore
/// let module = ScheduleModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::ScheduleModule) -> Router<()> {
    Router::new()
        .merge(create_schedule_routes(module.schedule_service.clone()))
        .merge(create_schedule_employee_routes(module.schedule_employee_service.clone()))
        .merge(create_schedule_organization_routes(module.schedule_organization_service.clone()))
        .merge(create_schedule_weekday_routes(module.schedule_weekday_service.clone()))
}

/// Read-only routes for the Schedule module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_schedule_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_schedule_routes(module: &crate::ScheduleModule) -> Router<()> {
    Router::new()
        .merge(create_schedule_read_routes(module.schedule_service.clone()))
        .merge(create_schedule_employee_read_routes(module.schedule_employee_service.clone()))
        .merge(create_schedule_organization_read_routes(module.schedule_organization_service.clone()))
        .merge(create_schedule_weekday_read_routes(module.schedule_weekday_service.clone()))
}

/// Get all routes (stateless) for the Schedule module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::ScheduleModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Schedule module.
///
...
pub fn create_combined_routes(module: &crate::ScheduleModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Schedule module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::ScheduleModule) -> Router<AppState> {
    Router::new()
