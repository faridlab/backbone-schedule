...
metaphor migration generate Invoice schedule
metaphor migration run
...
(`schedule` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed schedule          # run Rust seeders
metaphor migration generate-seeds schedule  # emit SQL seed files
```
